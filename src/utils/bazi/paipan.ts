/**
 * 从 zwd 中拿过来使用 TODO 抽离成独立包在两个项目中引入使用
 ...
 */
const Perturbation = function (jd) {
 ...
 * @return boolean
 */
const MeanJQJD = function (yy) {
  const jd = VE(yy);
  if (!jd) {
    //该年的春分點
    return [];
  }
  const ty = VE(yy + 1) - jd; //该年的回歸年長

  const num = 24 + 2; //另外多取2个确保覆盖完一个公历年
 ...
 * @return array
 */
const GetAdjustedJQ = function (yy, start, end) {
  if (start < 0 || start > 25) {
    return [];
  }
  if (end < 0 || end > 25) {
 ...
 */
const GetPureJQsinceSpring = function (yy) {
  const jdpjq = [];
...
const TrueNewMoon = function (k) {
  ...
  const t3 = t2 * t; //cube for frequent use
  const t4 = t3 * t; //to the fourth
  //mean time of phase
...
const MeanNewMoon = function (jd) {
  ...
  const t = (jdt - 2451545) / 36525; //以100年為單位,以2000年1月1日12時為0點
...
const GetZQandSMandLunarMonthCode = function (yy) {
  ...
  const jdnm = GetSMsinceWinterSolstice(yy, jdzq[0]); //求出以含冬至中氣為阴曆11月(冬月)開始的連續16個朔望月的新月點
...
class Paipan {
  ...
  ctg2 = ['甲木', '乙木', '丙火', '丁火', '戊土', '己土', '庚金', '辛金', '壬水', '癸水']; // 全称
  ...
  GetLunarDays(yy: number, mm: number, isLeap: boolean) {
    ...
    const lm = GetZQandSMandLunarMonthCode(yy);
    const jdzq = lm[0];
    const jdnm = lm[1];
    const mc = lm[2];

    let leap = 0; //若閏月旗標為0代表無閏月
    for (let j = 1; j <= 14; j++) {
      //確認指定年前一年11月開始各月是否閏月
  ...
  GetZodiac(mm: number, dd: number) {
    ...
    const dds = [20, 19, 21, 20, 21, 22, 23, 23, 23, 24, 22, 22]; //星座的起始日期
  ...
  GetWeek(yy: number, mm: number, dd: number) {
    const jd = Solar2Julian(yy, mm, dd, 12);
    if (!jd) {
  ...
  Solar2Lunar(yy: number, mm: number, dd: number) {
    ...
    let mi = 0;
    let prev = 0; //是否跨年了,跨年了则减一
    let isLeap = 0; //是否闰月

    let lm = GetZQandSMandLunarMonthCode(yy);
    let jdzq = lm[0];
    let jdnm = lm[1];
    let mc = lm[2];
  ...
  public Get24JieQiByYear(dateObj: Date) {
    ...
    if (
      dateObj.getTime() <
      new Date(jqs[0][0], jqs[0][1] - 1, jqs[0][2], jqs[0][3], jqs[0][4], jqs[0][5]).getTime()
    ) {
  ...
  GetGanZhi(yy: number, mm: number, dd: number, hh: number, mt = 0, ss = 0) {
    const jd = Solar2Julian(yy, mm, dd, hh, mt, Math.max(1, ss));
    if (!jd) {
      //多加一秒避免精度问题
    ...
    let jq = GetPureJQsinceSpring(yy); //取得自立春開始的节,该数组长度固定为16
    if (jd < jq[1]) {
      //jq[1]為立春,約在2月5日前後,
    ...
    const tmm = ((yy + 4712) * 12 + (ix - 1) + 60) % 60; //数组0为前一年的小寒所以这里再减一
    const mgz = (tmm + 50) % 60;
    tg[1] = mgz % 10; //月干
    dz[1] = mgz % 12; //月支

    const jda = jd + 0.5; //計算日柱之干支,加0.5是將起始點從正午改為從0點開始.
    const thes = (jda - Math.floor(jda)) * 86400 + 3600; //將jd的小數部份化為秒,並加上起始點前移的一小時(3600秒),取其整數值
    const dayjd = Math.floor(jda) + thes / 86400; //將秒數化為日數,加回到jd的整數部份
  ...
  public GetInfo(gender: 0 | 1, date: number) {
    ...
    let arr = [];
    const start_yy = res.yinli[0]; // 开始年份，以阴历年计算。因为如果生在阳历当年的立春前，此时阴历实际还属于前一年
    const [start_tg, start_dz] = this.getYearMidTgDz(start_yy); //  以年中为起点，防止生于两年之间导致第一年错误的问题
    for (let i = 0; i <= 130; i++) {
      ...
      i.days = days_JZ_60[index].map((j, k) => {
        ...
        if (k !== 0) {
          ...
          const [, , next_day_rizhu] = next_day_r.tg.map(
            (_, next_day_r_i) =>
              this.ctg[next_day_r.tg[next_day_r_i]] + this.cdz[next_day_r.dz[next_day_r_i]]
          );
  ...
  private getTenGodMap(self: number | string = '') {
    ...
    const self_index = (isYang ? this.yang : this.yin).findIndex((i) => i === self);
