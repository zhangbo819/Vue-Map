...
export const TG_10 = [TG.甲, TG.乙, TG.丙, TG.丁, TG.戊, TG.己, TG.庚, TG.辛, TG.壬, TG.癸];
...
export const YueClass5 = [YueClass.旺, YueClass.相, YueClass.休, YueClass.囚, YueClass.死];
...
type Xun = JZ_60.甲子 | JZ_60.甲午 | JZ_60.甲寅 | JZ_60.甲戌 | JZ_60.甲申 | JZ_60.甲辰;
...
class WuXingClass {
  ...
  public getXun(target: JZ_60): Xun {
    ...
    if (tg_index === 0) {
      ...
      const xun_dizhi_index =
        dz_index - tg_index < 0 ? dz_index - tg_index + DZ_12.length : dz_index - tg_index;
  ...
  private checkTGRelation(a: TG, b: TG) {
    ...
    for (const key in map) {
      map[key as TG_GX].forEach((gxItems, index) => {
        if ((gxItems[0] === a && gxItems[1] === b) || (gxItems[0] === b && gxItems[1] === a)) {
          if (key === TG_GX.克) {
  ...
  public getDzRelation(target: DZ[]) {
    ...
    for (let i = target.length - 1; i >= 0; i--) {
      ...
      for (let j = i - 1; j >= 0; j--) {
        ...
        // 3
        for (let k = j - 1; k >= 0; k--) {
          const gx3 = this.checkDZRelation(target[i], target[j], target[k], target);
          if (gx3.length) {
          ...
          if (four.includes(target[i]) && four.includes(target[j]) && four.includes(target[k])) {
            ...
            if (i_index === j_index || j_index === k_index || i_index === k_index) {
              continue;
            }
            ...
            for (let p = k - 1; p >= 0; p--) {
              if (target[p] === other) {
                relation.push({
                  ...
                  index: [p, k, j],
                  key: DZ_GX.四合,
                  text: this.map_dzgx[DZ_GX.四合][0].join('') + DZ_GX.四合 + '土局',
                  color: '',
                });
              }
            }
  ...
  // 校验地支关系
  private checkDZRelation(a: DZ, b: DZ, c: DZ | null = null, target: DZ[] = []) {
    ...
    for (const key in this.map_dzgx) {
      this.map_dzgx[key as DZ_GX].forEach((gxItems, index) => {
        if (key === DZ_GX.拱会 || key === DZ_GX.拱合 || key === DZ_GX.三刑之二) {
          ...
          [...gxItems].sort((x, y) => x.charCodeAt(0) - y.charCodeAt(0)).join('') === inputs
        ) {
          switch (key) {
            // TODO use color
            case DZ_GX.合:
            ...
            case DZ_GX.穿:
              ...
              res.push({
                key,
                text: gxItems[0] + DZ_GX.穿 + gxItems[1] + '(' + map_chuan[index] + ')',
                color: '',
              });
              break;
            case DZ_GX.刑:
  ...
  public getWxPower(bazi: JZ_60[], tenMap: Ten[]) {
    ...
    // 2. 成众判断
    const { wu_numbs } = this.getWxNumbs(bazi.map((i) => this.getWuxing(i[1]) as WX));
  ...
  private getWxSingleScore(wx: WX, bazi: JZ_60[], res: sizhuDetailsItem[]) {
    ...
    function getDeshiScore() {
      ...
      dz_gx.forEach((i) => {
        i.relation.forEach((relation) => {
          if (relation.text.includes(DZ_GX.三合) || relation.text.includes(DZ_GX.三会)) {
            const zhongshen = res.find(
              (j) => j.gen_dz.includes(relation.text[1] as DZ) // TODO index 1
            );
            if (zhongshen?.wx === wx) {
    ...
    function getTgScore() {
      ...
      const ke_wx = ke_map[(ke_map.findIndex((i) => i === wx) + 1) % ke_map.length];
      const sheng_map = [WX.金, WX.土, WX.火, WX.木, WX.水];
      const sheng_wx = sheng_map[(sheng_map.findIndex((i) => i === wx) + 1) % sheng_map.length];
    ...
    const res_score = Math.round(yuelingScore * (tgScore + deshiScore + dzScore + mkScore / 4 / 2));
  ...
  public getWxNumbs(wxs: WX[]) {
    ...
    const arr_wu_nums = Array(wxs.length)
      .fill(null)
      .map((_, index) => {
        const active = Object.keys(wu_numbs).filter((key) => wu_numbs[key as WX] === index);
        if (active.length) {
          return active;
        }
        return null;
  ...
  private getMingGong(yuegan: TG, yuezhi: DZ, shizhi: DZ) {
    ...
    if (mingGongIndex > yuezhi_index) {
      ...
      // 逆查天干
      mingGongStemIndex = (yuegan_index - (yuezhi_index - mingGongIndex) + 10) % 10;
    }
